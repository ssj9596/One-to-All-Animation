...
def compute_lpips(gen_inst_name_full, gt_inst_name_full,device='cuda'):
    gen_inst_name_full = sorted(gen_inst_name_full)
    gt_inst_name_full = sorted(gt_inst_name_full)
    ...
    for gen_path, gt_path in tqdm(zip(gen_inst_name_full, gt_inst_name_full)):   
        ...
        image1 = convert_tensor(load_image(gen_path)).unsqueeze(0).to(device)
        image2 = convert_tensor(load_image(gt_path)).unsqueeze(0).to(device)
    
        score = loss_fn_vgg(image1, image2).cpu().item()
