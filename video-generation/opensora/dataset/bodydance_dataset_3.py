...
class BodyDance_dataset_3(Dataset):
    def __init__(self, args, transform, temporal_sample, tokenizer, transform_topcrop, temporal_downscale, device='cuda', one_more_second=False, ignore_timestamps=False, valid=False, load_cached_latents=False, null_embedding_path=None):
        ...
        if args.change_pose > 0:
            self.pose_poolA = pd.read_csv(args.pose_poolA)
            self.pose_poolB = pd.read_csv(args.pose_poolB)
        else:
            self.pose_poolA = None
            self.pose_poolB = None

        self.change_pose = args.change_pose
        self.pose_aug = args.pose_aug
        self.ref_keep = args.ref_keep
        print("\n=== 参数配置 ===")
        ...
        print(f"self.change_pose: {args.change_pose}")
    ...
    def get_video(self, idx):
        ...
        video, pose, reference_frame, face_mask, ocr_mask, hand_mask, reference_pose_tensor, inpaint_mask = apply_random_cropping_strategy_v2(
            ...
            random_zoomin = self.video_crop,
            ref_keep = self.ref_keep,
        )
        ...
        return dict(pixel_values=video, caption=text, cfg_embed_scale = cfg_embed_scale, 
                        condition = condition, reference_frame = reference_frame, face_mask = face_mask, ocr_mask = ocr_mask, reference_pose=reference_pose_tensor)
    ...
    def get_image(self, idx):
        ...
        if random.random() < self.change_pose:
            aspect_ratio = ori_height / ori_width
            pool = self.pose_poolA if aspect_ratio >= 1 else self.pose_poolB
            row = pool.sample(1).iloc[0]
            rand_pose_path = row['pose_path']
            rand_h, rand_w = int(row['height']), int(row['width'])
            rand_arr  = np.load(rand_pose_path.replace(".npy", "_raw.npy"), allow_pickle=True)
            # change pose
            random_pose = build_pose_dict(rand_arr[0],rand_h, rand_w, aspect_ratio)
            new_pose_data = pose_data
            random_pose_aligned  = align_pose_by_scale_and_translation(new_pose_data[0], random_pose)
            # new_pose_data = align_poses_by_limb_scaling_only(random_pose_aligned, new_pose_data)
            new_pose_data, did_limb_change, did_face_change = align_poses_by_limb_scaling_only(
                                            random_pose_aligned, new_pose_data,
                                            limb_change_prob=0.4, 
                                            face_change_prob=0.7,   
                                        )
            pose = draw_pose_aligned(new_pose_data[0], ori_height, ori_width, without_face=without_face,head_change=did_limb_change,face_change=did_face_change)
        else:
            pose = draw_pose_aligned(pose_data[0], ori_height, ori_width, without_face=without_face)
            
        ref_pose = draw_pose_aligned(ref_pose_data[0], ori_height, ori_width, without_face=without_face)
        ...
        return dict(pixel_values=image, caption=text, cfg_embed_scale = cfg_embed_scale, 
                        condition = condition, reference_frame = reference_frame, face_mask = face_mask, ocr_mask = ocr_mask, reference_pose=ref_pose_tensor)
    ...
    def read_frames(self, idx, real_t):
        ...
        indices_to_build = set(train_indices)
        indices_to_build.add(ref_index)
        ...
        if random.random() < self.change_pose:
            ...
            # change pose
            random_pose = build_pose_dict(rand_arr[0],rand_h, rand_w, aspect_ratio)
            new_pose_data = [pose_data[i] for i in train_indices]
            random_pose_aligned  = align_pose_by_scale_and_translation(new_pose_data[0], random_pose)
            ...
            pose = get_batch_pose(new_pose_data, range(len(new_pose_data)), ori_height, ori_width,without_face, pose_aug_flag = self.pose_aug, head_change=did_limb_change,face_change=did_face_change)

            # debug
            if random.random() < 0.00:
                ori_img = video[0].copy()                     # (H,W,3)  uint8
                ori_pose_vis = draw_pose_aligned(
                    pose_data[train_indices[0]], ori_height, ori_width,
                    without_face=True,
                )                                             # (H,W,3)  uint8
                ref_pose_vis = draw_pose_aligned(
                    random_pose_aligned, ori_height, ori_width,
                    without_face=True,
                )
                aligned_pose_vis_0 = draw_pose_aligned(
                    new_pose_data[0], ori_height, ori_width,
                    without_face=True,
                )
                aligned_pose_vis_3 = draw_pose_aligned(
                    new_pose_data[3], ori_height, ori_width,
                    without_face=True,
                )
                aligned_pose_vis_5 = draw_pose_aligned(
                    new_pose_data[5], ori_height, ori_width,
                    without_face=True,
                )
                canvas = np.concatenate(
                    [ori_img, ori_pose_vis, ref_pose_vis, aligned_pose_vis_0, aligned_pose_vis_3, aligned_pose_vis_5], axis=1
                )
                # ---------- 保存 ----------
                os.makedirs('debug_vis', exist_ok=True)
                save_name = f'debug_vis/temp_{random.randint(0, 99):02d}.png'
                cv2.imwrite(save_name, cv2.cvtColor(canvas, cv2.COLOR_RGB2BGR))
                # print(f'[debug] save pose alignment vis -> {save_name}')


        else:
            without_face = random.random() < self.without_face
            pose = get_batch_pose(pose_data, train_indices, ori_height, ori_width,without_face,pose_aug_flag = self.pose_aug)


        reference_frame = decord_vr[ref_index].asnumpy()

        reference_pose = draw_pose_aligned(
                                            pose_data[ref_index],
                                            ori_height,
                                            ori_width,
