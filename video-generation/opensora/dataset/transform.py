...
#预处理方法: 先按照aspect_ratio进行centercrop，然后resize到target resolution
def get_video_transform(resize_height, resize_width, norm_fun):

    # update 0110: 16的整数倍
    resize_height = round(resize_height / 16) * 16
    resize_width = round(resize_width / 16) * 16
 
    resize = [CenterCropResizeVideo((resize_height, resize_width), interpolation_mode='nearest'), ]
...
# mask should only have 0 and 1
def get_mask_transform(resize_height, resize_width):
...
def resize(clip, target_size, interpolation_mode):
    ...
    if interpolation_mode == 'nearest':
        return torch.nn.functional.interpolate(clip, size=target_size)
    else:
...
class TemporalRandomCrop(object):
    ...
    def __call__(self, total_frames, target_frames):
        size = target_frames
