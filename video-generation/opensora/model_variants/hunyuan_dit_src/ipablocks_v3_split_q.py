...
class MMDoubleStreamBlockReFuser_V3(nn.Module):
    def __init__(
        ...
        # self.img_attn_q      = original_block.img_attn_q
        # self.img_attn_kv     = original_block.img_attn_kv
        ...
        # refuser
        self.img_refuser_q  = nn.Linear(original_block.hidden_size, original_block.hidden_size, bias=True)
        qk_norm_layer = get_norm_layer("rms")
        self.img_refuser_q_norm = (
            qk_norm_layer(original_block.hidden_size // original_block.heads_num, elementwise_affine=True, eps=1e-6,)
        )
    ...
    def forward(
        ...
        # refuser
        fuser_query = rearrange(self.img_refuser_q(img_modulated), "B L (H D) -> B L H D",H=self.heads_num)
        fuser_query = self.img_refuser_q_norm(fuser_query).to(img_v)
        ...
        fuser_query = apply_rotary_emb_single_ignore_t(fuser_query, freqs_cis, head_first=False)
...
class MMSingleStreamBlockReFuser_V3(nn.Module):
    def __init__(
        ...
        # split
        # self.linear1_q = original_block.linear1_q
        # self.linear1_kv_mlp = original_block.linear1_kv_mlp
        ...
        # self.refuser_q  = nn.Linear(original_block.hidden_size, original_block.hidden_size, bias=True)
        # qk_norm_layer = get_norm_layer("rms")
        # self.refuser_q_norm = (
        #     qk_norm_layer(original_block.hidden_size // original_block.heads_num, elementwise_affine=True, eps=1e-6)
        # )
    ...
    def forward(
        ...
        # fuser_query = rearrange(self.refuser_q(x_mod), "B L (H D) -> B L H D",H=self.heads_num)
        # fuser_query = self.refuser_q_norm(fuser_query).to(v)
        ...
        if freqs_cis is not None:
            ...
            # add here
            # img_fuser_q, txt_fuser_q = fuser_query[:, :-txt_len, :, :], fuser_query[:, -txt_len:, :, :]
            img_fuser_q, txt_fuser_q = q[:, :-txt_len, :, :], q[:, -txt_len:, :, :]
            img_fuser_q = apply_rotary_emb_single_ignore_t(img_fuser_q, freqs_cis, head_first=False)
            fuser_query = torch.cat((img_fuser_q, txt_fuser_q), dim=1)
