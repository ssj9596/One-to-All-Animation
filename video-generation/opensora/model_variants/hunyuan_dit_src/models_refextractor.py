...
class HYVideoDiffusionTransformer_Refextractor(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def setup_refuser(self, weight_dtype):
        ...
        if self.use_refuser is True:
            return 
        from .ipablocks import MMSingleStreamBlockReFuser_V2, MMDoubleStreamBlockReFuser_V2
        if len(self.refextractor.double_blocks) > 0:
            # ----  double_blocks ----
            new_double_blocks = []
            for i, block in enumerate(self.double_blocks):
                wrapped = MMDoubleStreamBlockReFuser_V2(
        ...
        if len(self.refextractor.single_blocks) > 0:
            ...
            for i, block in enumerate(self.single_blocks):
                wrapped_sblock = MMSingleStreamBlockReFuser_V2(
                    original_block=block,
                ).to(dtype=weight_dtype)
