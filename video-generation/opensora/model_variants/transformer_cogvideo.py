...
from .attention_processor import Attention, CogVideoXAttnProcessor2_0, FusedCogVideoXAttnProcessor2_0
...
def prepare_rotary_positional_embeddings(
    ...
    # grid_height = height // patch_size
    # grid_width = width // patch_size
    # base_size_width = base_width //  patch_size
    # base_size_height = base_height // patch_size
...
class CogVideoXTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def forward(
        ...
        image_rotary_emb = prepare_rotary_positional_embeddings(
                ...
                patch_size=2,
                patch_size_t = self.config.patch_size_t or 1,
        ...
        controlnet_states_index = []
        if controlnet_states is not None:
            controlnet_states = list(controlnet_states)
            interval_control, remainder = divmod(len(self.transformer_blocks)// 2, len(controlnet_states))
            start = len(self.transformer_blocks) // 2
            for _control_idx in range(len(controlnet_states)):
                if remainder > 0:
                    control_index = start + interval_control + 1
                    remainder -= 1
                else:
                    control_index = start + interval_control
                controlnet_states_index.append(start)
                start = control_index
        ...
        for i, block in enumerate(self.transformer_blocks):
            ...
            if (controlnet_states is not None) and (len(controlnet_states) > 0) and (i in controlnet_states_index):
                controlnet_states_block = controlnet_states.pop()
...
class CogVideoXTransformer3DModelControlnet(CogVideoXTransformer3DModel):
    def forward(
        self,
        hidden_states: torch.Tensor,
        encoder_hidden_states: torch.Tensor,
