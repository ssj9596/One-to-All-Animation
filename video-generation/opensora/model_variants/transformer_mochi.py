...
class MochiTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        temb, encoder_hidden_states = self.time_embed( #类似sd3中的操作，这里使用的应该是global pooling的方法，然后和原始的输入计算一个cross attention
            timestep, encoder_hidden_states, encoder_attention_mask, hidden_dtype=hidden_states.dtype
        )
        hidden_states = hidden_states.flatten(0, 1)
        hidden_states = self.patch_embed(hidden_states) #简单的patch embedding，类似Opensora_plan
        hidden_states = hidden_states.unflatten(0, (batch_size, -1)).flatten(1, 2)

        image_rotary_emb = self.rope( #这里也是在一个192x192的grid上进行类似crop的形式来缩放的，但是这里更加简单，直接根据面积比来进行相对scale的转换
