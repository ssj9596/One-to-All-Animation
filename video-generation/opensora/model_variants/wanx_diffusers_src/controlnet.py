...
class WanControlNet(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        try:
            from flash_attn_interface import flash_attn_func as flash_attn3_func
            # flash attn3 {"use_reentrant": True}
            ...
            self._gradient_checkpointing_func_flash_attn3 = _gradient_checkpointing_func
            print("set {use_reentrant: True}")
            # self._set_gradient_checkpointing(enable=True, gradient_checkpointing_func=gradient_checkpointing_func)
        except:
            self._gradient_checkpointing_func_flash_attn3 = None
            pass
    ...
    def forward(
        ...
        for block in self.blocks:
            ...
            # no gradient_checkpointing to speed up!
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                hidden_states = self._gradient_checkpointing_func_flash_attn3(
                    block, hidden_states, encoder_hidden_states, timestep_proj, rotary_emb
                ) if self._gradient_checkpointing_func_flash_attn3 is not None else self._gradient_checkpointing_func(
                    block, hidden_states, encoder_hidden_states, timestep_proj, rotary_emb
                )
            else:
