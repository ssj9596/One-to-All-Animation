...
def apply_rotary_emb_split(hidden_states, freqs, t_dim):
    t_part, hw_part = torch.split(hidden_states, [t_dim, hidden_states.shape[-1] - t_dim], dim=-1)
    hw_freqs = freqs[..., t_dim//2:]

    x_hw = torch.view_as_complex(hw_part.to(torch.float64).unflatten(-1, (-1, 2)))
    out_hw = torch.view_as_real(x_hw * hw_freqs).flatten(-2, -1)
    return torch.cat([t_part, out_hw.type_as(hidden_states)], dim=-1)
...
class WanAttnProcessor2_0:
    ...
    def __call__(
        ...
        if rotary_emb is not None:
            ...
            # t_dim不做rope!!
            query = apply_rotary_emb_split(query, rotary_emb, t_dim)
            key = apply_rotary_emb_split(key, rotary_emb,t_dim)
...
class WanRefextractor(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def forward(
        ...
        # if ref_pose_cond is not None and add_pose:
        #     hidden_states = hidden_states + ref_pose_cond
