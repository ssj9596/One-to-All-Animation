...
class OpenSoraT2V(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        """
        batch_size, c, frame, h, w = hidden_states.shape # [16, 4, 8, 60, 80]
        ...
        if attention_mask is not None and attention_mask.ndim == 4:
            ...
            # b, 1, h, w -> only images
            attention_mask = attention_mask.to(self.dtype) # [16, 8, 60, 80]nnu
        ...
        ) # hidden_states_vid[16, 9600, 2880]其中2880是dim，9600=8x30x40是序列长度
        # 2. Blocks
        # import ipdb;ipdb.set_trace()
        if get_sequence_parallel_state():
        ...
        for block in self.transformer_blocks:
            if self.training and self.gradient_checkpointing:
                ...
                if hidden_states_vid is not None:
                    hidden_states_vid = block( # hidden_states_vid[16,9600,2880]输入和输出一致
                        hidden_states_vid,
        ...
        if hidden_states_vid is not None:
            output_vid = self._get_output_for_patched_inputs( # output_vid[16, 4, 8, 60, 80]
...
def OpenSoraT2V_ROPE_L_4B_122(**kwargs):
    return OpenSoraT2V(num_layers=32, attention_head_dim=120, num_attention_heads=24, patch_size_t=1, patch_size=2,
                       norm_type="ada_norm_single", caption_channels=4096, cross_attention_dim=2880, **kwargs)

def OpenSoraT2V_ROPE_L_5B_122(**kwargs):
    return OpenSoraT2V(num_layers=40, attention_head_dim=120, num_attention_heads=24, patch_size_t=1, patch_size=2,
                       norm_type="ada_norm_single", caption_channels=4096, cross_attention_dim=2880, **kwargs)

def OpenSoraT2V_ROPE_L_6B_122(**kwargs):
    return OpenSoraT2V(num_layers=32, attention_head_dim=144, num_attention_heads=24, patch_size_t=1, patch_size=2,
                       norm_type="ada_norm_single", caption_channels=4096, cross_attention_dim=3456, **kwargs)

OpenSora_models = {
    "OpenSoraT2V-S/122": OpenSoraT2V_S_122,  # 1.1B
    "OpenSoraT2V-B/122": OpenSoraT2V_B_122, # 1.27B
    "OpenSoraT2V-L/122": OpenSoraT2V_L_122, # 2.7B
    "OpenSoraT2V-ROPE-L/122": OpenSoraT2V_ROPE_L_122, # 2.7B
    "OpenSoraT2V-ROPE-L-4B/122": OpenSoraT2V_ROPE_L_4B_122, # 4.32B
    "OpenSoraT2V-ROPE-L-5B/122": OpenSoraT2V_ROPE_L_5B_122, # 5.38B
    "OpenSoraT2V-ROPE-L-6B/122": OpenSoraT2V_ROPE_L_6B_122, # 6.22B
}
...
OpenSora_models_class = {
    ...
    "OpenSoraT2V-ROPE-L-4B/122": OpenSoraT2V,
    "OpenSoraT2V-ROPE-L-5B/122": OpenSoraT2V,
    "OpenSoraT2V-ROPE-L-6B/122": OpenSoraT2V,
}
...
if __name__ == '__main__':
    ...
    cond_c = 4096
    num_timesteps = 1000
    ae_stride_t, ae_stride_h, ae_stride_w = ae_stride_config[args.ae] # 4, 8, 8
    latent_size = (args.max_height // ae_stride_h, args.max_width // ae_stride_w)
    num_frames = (args.num_frames - 1) // ae_stride_t + 1 # 8

    device = torch.device('cuda:0')
    # model = OpenSoraT2V_ROPE_L_122(in_channels=c, 
    model = OpenSoraT2V_ROPE_L_5B_122(in_channels=c, 
                              out_channels=c, 
