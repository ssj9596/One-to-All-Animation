...
class PatchEmbed2D(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("pos_embed", torch.from_numpy(pos_embed).float().unsqueeze(0), persistent=False) #persistent=False，不会在ckpt中被保留，因此每次都是全新的
    ...
    def forward(self, latent, num_frames):
        ...
        if self.use_abs_pos: #不会走这里，应该是使用了ROPE
        ...
        try:
            ...
            if attention_mask is not None:
                if npu_config is None:
                    ...
                    if get_sequence_parallel_state():
                        ...
                        attention_mask = attn.prepare_attention_mask(attention_mask, sequence_length * 0, batch_size)
            ...
            if npu_config is not None and npu_config.on_npu:
                ...
                if get_sequence_parallel_state():
                    ...
                    if self.attention_mode == 'flash' and attention_mask is None:
                        with torch.backends.cuda.sdp_kernel(enable_math=False, enable_flash=True, enable_mem_efficient=False):
                            hidden_states = F.scaled_dot_product_attention(
                                query, key, value, dropout_p=0.0, is_causal=False
                            )
                    elif (self.attention_mode == 'xformers') or (self.attention_mode == 'flash' and attention_mask is not None):
                    ...
                    query = query.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2) #这里transpse了一下，没问题
            ...
            print([hidden_states.shape, encoder_hidden_states.shape, query.shape, key.shape, value.shape, attention_mask.shape])
            raise Exception(e)
