...
from opensora.trainer_misc import (
...
class PyramidDiffusionMMDiT(ModelMixin, ConfigMixin):
    ...
    def merge_input(self, sample, encoder_hidden_length, encoder_attention_mask):
        ...
        num_stages = len(sample) #看上去这里始终是1
        ...
        if self.pos_embed_type == 'rope':
            ...
            if self.temp_pos_embed_type == 'rope' and self.add_temp_pos_embed:
                #默认在这
                image_ids_list = self._prepare_pyramid_temporal_rope_ids(sample, pad_batch_size, device) #每一帧的所有token给一个rope的id，看上去是给每一帧的每个token一个时序的frame idx
                text_ids = torch.zeros(pad_batch_size, encoder_attention_mask.shape[1], 1).to(device=device) #给encoder hidden stages的每一个token一个时间上的idx   
                input_ids_list = [torch.cat([text_ids, image_ids], dim=1) for image_ids in image_ids_list] #将文本的token和image的token concat到一起
                image_rotary_emb = [self.temp_rope_embed(input_ids) for input_ids in input_ids_list]  # [bs, seq_len, 1, head_dim // 2, 2, 2] 简单地计算1D token
        ...
        if self.use_flash_attn:
            ...
            #就是给每个token找一个idx的方式来确定attention mask。当num_stages>1的时候，要注意repeat的方式，比如batch_size=2, num_stages=3, 则real_batch_size=6, 用repeat的方式扩展的话，则6中的123来自第一个sample，456来自第二个sample，因此这里1 4配对，2 5配对
            assert encoder_attention_mask.shape[1] == encoder_hidden_length
            real_batch_size = encoder_attention_mask.shape[0] #理解成这里的batch是batch_size * num_stages
            # prepare text ids
            ...
            for i_p in range(len(hidden_length)):
                ...
                token_ids = torch.cat([text_ids[i_p::num_stages], image_ids], dim=1) #文本的token在image的token前面
    ...
    def forward(
        self,
        sample: torch.FloatTensor, # [num_stages] # sample其实是一个list
        ...
        temb = self.time_text_embed(timestep_ratio, pooled_projections) #timestep的embedding和text的embedding求和得到condition，timestep的embedding和DDPM中相同，为sincos embedding
        encoder_hidden_states = self.context_embedder(encoder_hidden_states) #linear
