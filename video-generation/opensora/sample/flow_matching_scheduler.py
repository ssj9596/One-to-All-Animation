'''
the standard score matching but support pyramid flow matching
'''
...
class PyramidFlowMatchEulerDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def init_sigmas_for_each_stage(self):
        ...
        for i_s in range(stages):
            ...
            if not self.invert_sigmas:
                end_sigma = self.sigmas[end_indice].item() if end_indice < training_steps else 0.0
            else:
                end_sigma = self.sigmas[end_indice].item() if end_indice < training_steps else 1.0
            self.ori_start_sigmas[i_s] = start_sigma

            if i_s != 0:
                #这里应该是这里的t的定义其实是论文中的1-t
                if not self.invert_sigmas:
                    ori_sigma = 1 - start_sigma
                else:
                    ori_sigma = start_sigma
                gamma = self.config.gamma
                corrected_sigma = (1 / (math.sqrt(1 + (1 / gamma)) * (1 - ori_sigma) + ori_sigma)) * ori_sigma # Eq 25
                if not self.invert_sigmas:
        ...
        # NOTE：注意这里的计算。这里之所以需要计算ratio和训练方式相关。这里计算的是一个全局的timesteps，等价于每个stage分别训练了一个模型，用timesteps区分不同的模型。sigma和timesteps不再是可以相互计算的
        # 对于我们训练了一个一般的flow-matching模型，sigma回退的时候，timesteps也需要进行回退
        ...
        # for i_s in range(stages):
            ...
            #     timestep_max, timestep_min, training_steps + 1, #加1是最后一个step是最后的step
            # )
            # self.timesteps_per_stage[i_s] = timesteps[:] if isinstance(timesteps, torch.Tensor) else torch.from_numpy(timesteps[:])
            # start_sigma = self.start_sigmas[i_s]
            # end_sigma = self.end_sigmas[i_s]
            # stage_sigmas = np.linspace(
            #     start_sigma, end_sigma, training_steps,
            # )
            # self.sigmas_per_stage[i_s] = torch.from_numpy(stage_sigmas[:])
    ...
    def set_timesteps(self, num_inference_steps: int, stage_index: int, device: Union[str, torch.device] = None):
        ...
        sigma_max = self.start_sigmas[stage_index]
        sigma_min = self.end_sigmas[stage_index]

        ratios = np.linspace(
            sigma_max, sigma_min, num_inference_steps+1
        )
        self.sigmas = torch.from_numpy(ratios).to(device=device)
        self.timesteps = self.sigmas * training_steps
        invert_sigmas = True
        sigmas = self.sigmas
        # if invert_sigmas:
        #     sigmas = 1.0 - sigmas
