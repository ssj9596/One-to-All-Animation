...
from opensora.utils.utils import GaussianNoiseAdder, numpy_to_pil, black_image
...
class HunyuanVideoPipeline(DiffusionPipeline, HunyuanVideoLoraLoaderMixin):
    ...
    def prepare_latents(
        ...
        if initial_image is not None:
            ...
            if initial_image.shape[2] > 1: # [past_image_latents, image]
                image_latents = torch.cat([self.vae.encode(initial_image[:,:,0:-1,...]).latent_dist.sample(), self.vae.encode(initial_image[:,:,-1:,...]).latent_dist.sample()], dim = 2)
            else:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i2v_type == "concat":
                    ...
                    latents = self.scheduler.step(
                        noise_pred[:,:,num_pre:,...], t, latents[:,:,num_pre:,...], **extra_step_kwargs, return_dict=False
                    )[0]
