...
from opensora.train.edm import EDMPrecond
...
class OpenSoraEDMPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        **kwargs,
        ...
        '''
        EDM sampler hyper-parameters, ref to EDM paper
        '''
        edm_model_wrapper = EDMPrecond()
        ...
        if not self.use_flow:
            ...
            rho = 1 # setting rho=1 equals decrease step size motonically with sigma in flow sampling
        S_churn = 0
        S_min = 0
        S_max = float('inf')
        S_noise = 1
        ...
        # 4. Prepare timesteps
        sigma_min = max(sigma_min, edm_model_wrapper.sigma_min)
        sigma_max = min(sigma_max, edm_model_wrapper.sigma_max)
        step_indices = torch.arange(num_inference_steps, dtype=torch.float64, device=device)
        t_steps = (sigma_max ** (1 / rho) + step_indices / (num_inference_steps - 1) * (sigma_min ** (1 / rho) - sigma_max ** (1 / rho))) ** rho #非均匀step_size
        t_steps = torch.cat([edm_model_wrapper.round_sigma(t_steps), torch.zeros_like(t_steps[:1])]) # t_N = 0 实际上t就是sigma，最后一个sigma是0
        ...
        with self.progress_bar(total=num_inference_steps-1) as progress_bar:
            for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
                x_cur = latents
                if not self.use_flow and False:
                    gamma = min(S_churn / num_inference_steps, np.sqrt(2) - 1) if S_min <= t_cur <= S_max else 0 
                else:
                    gamma = 0

                t_hat = edm_model_wrapper.round_sigma(t_cur + gamma * t_cur)
                ...
                latent_model_input = torch.cat([x_hat] * 2) if do_classifier_free_guidance else latents
                ...
                model = partial(self.transformer, **model_kwargs)
                noise_pred = edm_model_wrapper.forward(model, latent_model_input/s_t(t_hat), sigma_t(t_hat).repeat(latent_model_input.shape[0], 1)).to(torch.float64)
                ...
                d_cur = (x_hat - noise_pred) / t_hat
                x_next = x_hat + (t_next - t_hat) * d_cur
                if i < num_inference_steps - 1 and not self.use_flow:
                    latent_model_input = torch.cat([x_next] * 2) if do_classifier_free_guidance else latents
                    noise_pred = edm_model_wrapper.forward(model, latent_model_input/s_t(t_next), sigma_t(t_next).repeat(latent_model_input.shape[0], 1)).to(torch.float64)
                    if do_classifier_free_guidance:
                        noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
                        noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond)
                    d_prime = (x_next - noise_pred) / t_next
