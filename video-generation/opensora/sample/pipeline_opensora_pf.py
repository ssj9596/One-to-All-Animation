...
class PFOpenSoraPipeline(DiffusionPipeline):
    r"""
    ...
    def prepare_latents(self, batch_size, num_channels_latents, num_frames, height, width, dtype, device, generator, latents=None):
        shape = (
            ...
            math.ceil(int(height) / self.vae.vae_scale_factor[1]) + math.ceil(int(height) / self.vae.vae_scale_factor[1]) % 2,
            math.ceil(int(width) / self.vae.vae_scale_factor[2]) + math.ceil(int(width) / self.vae.vae_scale_factor[2]) % 2,
        )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        ori_height = height
        ori_width = width
        resize_scale = 2**(stages - 1)
        height, width = int(ori_height/resize_scale), int(ori_width/resize_scale)
        ...
        for i_s in range(stages):
            ...
            if i_s > 0:
                ...
                latents = F.interpolate(latents, size=(height, width), mode='nearest') #NOTE：这里是在latent上进行上采样的
        ...
        if get_sequence_parallel_state():
            ...
            torch.distributed.all_gather_into_tensor(all_latents, latents.contiguous())
