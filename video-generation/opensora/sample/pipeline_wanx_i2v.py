...
class WanImageToVideoPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def encode_image(
        ...
        device = device or self._execution_device
        # <PIL.Image.Image image mode=RGB size=384x672 at 0x7FD79CF99FC0>
        # add crop
    ...
    def prepare_latents(
        ...
        if mode == "drop_mask":
            return latents, latent_condition.repeat(1,1,num_latent_frames,1,1)
        # import pdb; pdb.set_trace()

        mask_lat_size = torch.ones(batch_size, 1, num_frames, latent_height, latent_width)
        mask_lat_size[:, :, list(range(1, num_frames))] = 0
        first_frame_mask = mask_lat_size[:, :, 0:1]
        # repeat * 4 torch.Size([1, 1, 4, h, w]) ones
        first_frame_mask = torch.repeat_interleave(first_frame_mask, dim=2, repeats=self.vae_scale_factor_temporal)
        mask_lat_size = torch.concat([first_frame_mask, mask_lat_size[:, :, 1:, :]], dim=2)
        # 4 + num_frames - 1, num_frames = 4*(latent_t-1） + 1
    ...
    def __call__(
        ...
        image = self.video_processor.preprocess(image, height=height, width=width).to(device, dtype=self.vae.dtype) # 原来是fp32
