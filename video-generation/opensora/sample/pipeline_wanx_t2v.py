            ...
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance:
                    noise_uncond = self.transformer(
                        ...
                        return_dict=False,
                        conditioning_scale = controlnet_conditioning_scale,
                        image_latents=black_image_latents if black_image_cfg else image_latents,
                    )[0]

                    if use_cfg_zero_star:
                        # https://arxiv.org/abs/2503.18886 
                        positive_flat = noise_pred.view(batch_size, -1)  # [B, C*T*H*W]
                        negative_flat = noise_uncond.view(batch_size, -1)
                        alpha = (positive_flat * negative_flat).sum(dim=1) / (negative_flat * negative_flat).sum(dim=1)
                        alpha = alpha.view(batch_size, *([1] * (len(noise_pred.shape) - 1)))
                        alpha = alpha.to(noise_pred.dtype)

                        if (i <= zero_steps) and use_zero_init:
                            noise_pred = noise_pred * 0.0 
                        else:
                            noise_pred = noise_uncond * alpha + guidance_scale * (noise_pred - noise_uncond * alpha)
