...
class WanPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def prepare_latents(
        ...
        return latents, image_latents, black_image_latents
    ...
    def __call__(
        ...
        latents, image_latents, black_image_latents = self.prepare_latents(
    ...
    def flowedit(
        ...
        source_image = None,
        target_image = None,
        source_control_video = None,
        target_control_video = None,
        controlnet_conditioning_scale = 0,
        negative_prompt: Union[str, List[str]] = None,
        height: int = 480,
        width: int = 832,
        num_frames: int = 81,
        num_inference_steps: int = 50,
        source_guidance_scale: float = 5.0,
        target_guidance_scale: float = 5.0,
        num_videos_per_prompt: Optional[int] = 1,
        generator: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
        latents: Optional[torch.Tensor] = None,
        output_type: Optional[str] = "np",
        ...
        device = self._execution_device
        # source_condition
        source_video = source_video.to(device, dtype=self.vae.dtype)

        source_image = self.video_processor.preprocess(source_image, height=height, width=width).to(device, dtype=self.vae.dtype)
        source_image = source_image.unsqueeze(2)
        source_control_video = source_control_video.to(device, dtype=self.transformer.dtype)
        source_prompt_embeds, negative_prompt_embeds = self.encode_prompt(
        ...
        source_prompt_embeds = source_prompt_embeds.to(self.transformer.dtype)
        negative_prompt_embeds = negative_prompt_embeds.to(self.transformer.dtype)

        # target_condition
        target_control_video = target_control_video.to(device, dtype=self.transformer.dtype)
        target_image = self.video_processor.preprocess(target_image, height=height, width=width).to(device, dtype=self.vae.dtype)
        ...
        target_prompt_embeds = target_prompt_embeds.to(self.transformer.dtype)

        # prepare init_latent source_img_latent and target_img_latent
        latents_mean = (
            torch.tensor(self.vae.config.latents_mean)
            .view(1, self.vae.config.z_dim, 1, 1, 1)
        ...
        # video
        init_latents = retrieve_latents(self.vae.encode(source_video), sample_mode="argmax")
        init_latents = (init_latents - latents_mean) * latents_std
        init_latents = init_latents.to(self.transformer.dtype)

        source_image_latents = retrieve_latents(self.vae.encode(source_image), sample_mode="argmax")
        source_image_latents = (source_image_latents - latents_mean) * latents_std
        source_image_latents = source_image_latents.to(self.transformer.dtype)

        target_image_latents = retrieve_latents(self.vae.encode(target_image), sample_mode="argmax")
        target_image_latents = (target_image_latents - latents_mean) * latents_std
        target_image_latents = target_image_latents.to(self.transformer.dtype)

        black_pil = Image.new("RGB", (width, height), color=(0, 0, 0))
        black_tensor = self.video_processor.preprocess(black_pil, height=height, width=width)
        black_tensor = black_tensor.to(device=device, dtype=self.vae.dtype).unsqueeze(2)
        black_image_latents = retrieve_latents(self.vae.encode(black_tensor), sample_mode="argmax")
        black_image_latents = (black_image_latents - latents_mean) * latents_std
        black_image_latents = black_image_latents.to(self.transformer.dtype)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for idx, (t, t_prev) in enumerate(zip(timesteps[:-1], timesteps[1:])):
                ...
                noise = randn_tensor(x_init.shape, generator=generator, device=device, dtype=self.transformer.dtype)
                sigma = t / 1000.0
                sigma_prev = t_prev / 1000.0

                timestep = t.expand(x_init.shape[0])

                # 加噪
                zt_src = (1 - sigma) * x_init + sigma * noise

                if idx < self._num_timesteps - drift_steps:
                    # 源点对齐（把 src 的变化搬到 tgt 上）
                    zt_tgt = x_tgt + (zt_src - x_init)

                    vt_src = self.calculate_vt(
                        hidden_states=zt_src,
                        controlnet_cond=source_control_video,
                        timestep=timestep,
                        prompt_embeds=source_prompt_embeds,
                        negative_prompt_embeds = negative_prompt_embeds,
                        attention_kwargs=attention_kwargs,
                        conditioning_scale=controlnet_conditioning_scale,
                        image_latents=source_image_latents,
                        black_image_latents=black_image_latents,
                        return_dict=False,
                ...
                vt_tgt = self.calculate_vt(
                    hidden_states=zt_tgt,
                    controlnet_cond=target_control_video,
                    timestep=timestep,
                    prompt_embeds=target_prompt_embeds,
                    negative_prompt_embeds = negative_prompt_embeds,
                    attention_kwargs=attention_kwargs,
                    conditioning_scale=controlnet_conditioning_scale,
                    image_latents=target_image_latents,
                    black_image_latents=black_image_latents,
                    return_dict=False,
                ...
                # 更新残差
                v_delta = vt_tgt - vt_src if isinstance(vt_src, torch.Tensor) else vt_tgt
                x_tgt = x_tgt + (sigma_prev - sigma) * v_delta
                # x_tgt = x_tgt.to(torch.float32)


                if idx == len(timesteps) - 1 or ((idx + 1) > num_warmup_steps and (idx + 1) % self.scheduler.order == 0):
                    progress_bar.update()



        self._current_timestep = None
        latents = x_tgt.to(self.transformer.dtype)
