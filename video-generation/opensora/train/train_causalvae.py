...
def remove_extra_checkpoints(checkpoint_dir):
    filelists = os.listdir(checkpoint_dir)
    filelists = sorted(filelists, key = lambda x: int(os.path.splitext(x)[0].split('-')[1]))
    if len(filelists) > 2:
        for i in range(0, len(filelists)-2):
            os.remove(os.path.join(checkpoint_dir, filelists[i]))
...
def save_checkpoint(
    ...
    filepath_override = '/tmp/checkpoint.ckpt'
    ...
    shutil.copy(filepath_override, filepath)
    return filepath
...
def train(args):
    ...
    is_master = node_rank == 0 and rank == 0
    ...
    if args.resume_from_checkpoint:
        ...
        if "ema_state_dict" in checkpoint and len(checkpoint['ema_state_dict']) > 0 and os.environ.get("NOT_USE_EMA_MODEL", 0) == 0:
            ...
            # model.module.load_state_dict(checkpoint["ema_state_dict"])
            ckpt = {key.replace("module.", ""): value for key, value in checkpoint["ema_state_dict"].items()}
            missing_keys, unexpected_keys = model.module.load_state_dict(ckpt, strict=True)
        else:
            missing_keys, unexpected_keys = model.module.load_state_dict(checkpoint["state_dict"]["gen_model"], strict=True)
        print(f"missing_keys: {len(missing_keys)}, unexpected_keys: {len(unexpected_keys)}")
        disc.module.load_state_dict(checkpoint["state_dict"]["dics_model"], strict=True)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if step_gen:
                ...
                if args.ema:
                    ema.update()
                if is_master and current_step % args.log_steps == 0:
            ...
            def valid_model(model, name=""):
                ...
                if rank == 0:
                    ...
                    print(f"psnr: {valid_psnr}   lpips: {valid_lpips}")
            ...
            # Checkpoint
            if current_step % args.save_ckpt_step == 0 and is_master:
                try:
                    remove_extra_checkpoints(ckpt_dir)
                    ...
                    logger.info(f'caught error {e} while saving ckpts, continue running')
