...
"""
A minimal training script for controlnet using PyTorch DDP.
"""
import argparse
import logging
import math
...
def custom_models(args, weight_dtype):
    ...
    controlnet.gradient_checkpointing = args.gradient_checkpointing
    try:
        controlnet.enable_gradient_checkpointing()
    except Exception as e:
        print(e)
    assert controlnet.gradient_checkpointing
    ...
    return text_encoder, vae, transformer, model_cls, controlnet, controlnet_model_cls
...
def main(args):
    ...
    args.valid=False
    train_dataset = getdataset(args, accelerator.device)
    ...
    valid_dataset = getdataset(valid_args, accelerator.device)
    ...
    text_enc, vae, model, dit_model_cls, controlnet, controlnet_model_cls = custom_models(args, weight_dtype)
    # use pretrained dit?
    if args.pretrained:
    ...
    if args.pretrained_controlnet:
        ...
        try:
            missing_keys, unexpected_keys = controlnet.load_state_dict(checkpoint, strict=False)
        except Exception as e:
            print("!!!!!!!! error in load pretrained ckpt:", e)
        logger.info(f'missing_keys {len(missing_keys)} {missing_keys}, unexpected_keys {len(unexpected_keys)}')
        logger.info(f'Successfully load {len(model_state_dict) - len(missing_keys)}/{len(model_state_dict)} keys from {args.pretrained_controlnet}!')
    ...
    cotracker = torch.hub.load("facebookresearch/co-tracker", "cotracker3_offline").to(accelerator.device).eval()
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                if args.use_ema:
                    ema_model.save_pretrained(os.path.join(output_dir, "controlnet_ema"))
        ...
        def load_model_hook(models, input_dir): #当使用deepspeed进行训练的时候，这里models是空的
            if args.use_ema:
                load_model = EMAModel.from_pretrained(os.path.join(input_dir, "controlnet_ema"), controlnet_model_cls)
            ...
            for i in range(len(models)):
                ...
                model = models.pop()
                # load diffusers style into controlnet
    ...
    logger.info(f"  Total training parameters = {sum(p.numel() for p in controlnet.parameters() if p.requires_grad) / 1e9} B")
    controlnet, optimizer, train_dataloader, lr_scheduler, valid_dataloader = accelerator.prepare(
        controlnet, optimizer, train_dataloader, lr_scheduler, valid_dataloader
    )
    ...
    def run(model_input, condition, model_kwargs, prof):
        ...
        loss = diffusion.get_loss_controlnet(condition, controlnet, model, model_input, model_kwargs, args, accelerator = accelerator, stage_index = stage_index)
        ...
        if accelerator.sync_gradients:# 预期上，当设置了accum的时候，只有当进行参数更新的时候才会进入
            ...
            if (progress_info.global_step == 1 or progress_info.global_step % args.evaluation_every == 0) and hasattr(diffusion, 'get_valid_loss_controlnet'):
                #多卡进行validation
                if accelerator.is_local_main_process:
                    logger.info("==================> doing validation")
                validation_runner.run_valid_controlnet(model, vae, text_enc, controlnet, cotracker, diffusion, valid_dataloader, accelerator, weight_dtype, args, logger)
    ...
    def train_one_step(step_, data_item_, prof_=None):
        ...
        if condition:
            ...
            for i in range(_x.shape[0]):
                _ = get_condition(_x[i], cotracker, accelerator.device, args) # n t h w
...
if __name__ == "__main__":
    ...
    # control net
    parser.add_argument("--load_condition", type=bool, required=True, default=True)
    parser.add_argument("--condition_dim", type=int, required=True, default=8)
    parser.add_argument("--init_from_transformer", type=bool, required=True, default=True)
    parser.add_argument("--controlnet_weights", type=float, required=True, default=1)
    parser.add_argument("--controlnet_name", type=str, required=True, default='cogvideo-controlnet')
    parser.add_argument("--sample_type", type=str, required=True, default="random", help="random, grid, sparse")
    # dataset & dataloader
    ...
    parser.add_argument("--controlnet_config_path", type=str, default="stabilityai/sd-vae-ft-mse")
    ...
    parser.add_argument("--dit_name", choices=['cogvideo', 'mochi', "hunyuan", "pyramid_flow","cogvideo-controlnet","hunyuan-controlnet"])
