...
from opensora.schedulers import RFLOW
import wandb
    ...
    def train_one_step(step_, data_item_, prof_=None):
        ...
        if get_sequence_parallel_state():
            x, cond, attn_mask, cond_mask, use_image_num = prepare_parallel_data(x, cond, attn_mask, cond_mask,
                                                                                 args.use_image_num)
            for iter in range(args.train_batch_size * args.sp_size // args.train_sp_batch_size): # train_batch_size=1, sp_size=8 train_sp_batch_size=2 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reflow", action="store_true",
                        help=(
                            "Whether or not to allow rectify flow."
                        ),
                        )
    parser.add_argument(
        "--weighting_scheme",
