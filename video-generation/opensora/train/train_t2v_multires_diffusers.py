...
def main(args):
    ...
    # sampler = LengthGroupedSampler( #简单地将数据按照world_size, batch_size划分index list，如果有multi_frame训练，则每个batch取frequency最高的那个帧数，然后不足batch_size数量的，随机过采样补齐
    ...
    #     # pin_memory=True,
    #     collate_fn=Collate(args), #collate: 做了一些操作，主要是如果视频的长宽不能整除之类的，进行padding，然后计算attention mask。这些部分实际上不必要，因为可以在dataset中预设好分辨率，避免即使padding之类的操作
    ...
    def train_all_epoch(prof_=None):
        for epoch in range(first_epoch, args.num_train_epochs):
            ...
            if progress_info.local_step > 0:
                print('starting from a middle step')
