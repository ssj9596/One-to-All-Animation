...
def main(args):
    ...
    if not args.frozen_refextractor:
    ...
    def train_one_step(step_, data_item_, prof_=None):
        ...
        x, caption, batch_embed_scale, condition, reference_frame, face_mask, ocr_mask = data_item_
        ...
        with accelerator.accumulate(model):
            ...
            if args.task == 't2v':
                if reference_frame_mask is not None:
                    ...
                    # reference_frame_mask = F.interpolate(reference_frame_mask.squeeze(2),size=target_spatial_size,mode='nearest').unsqueeze(2).to(weight_dtype)
                    reference_frame_mask = vae.encode(reference_frame_mask).to(weight_dtype) # 16channel
                ...
                model_kwargs = dict(encoder_hidden_states=cond, batch_embed_scale=batch_embed_scale,
                                    image_emb=image_emb,image_latents=image_latents,face_mask=face_mask,ocr_mask=ocr_mask,conditioning_scale=0,token_replace_prob=args.token_replace_prob)
