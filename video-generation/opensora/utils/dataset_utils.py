...
class Collate:
    ...
    def package(self, batch):
        ...
        batch_reference_frame = [i['reference_frame'] for i in batch]
        batch_embed_scale = [i['cfg_embed_scale'] for i in batch]
        batch_conditions = [i['condition'] for i in batch]
        batch_face_mask = [i['face_mask'] for i in batch]
        batch_ocr_mask = [i['ocr_mask'] for i in batch]

        if isinstance(batch[0]['caption'], dict):
            caption = []
            for k in list(batch[0]['caption'].keys()):
                if batch[0]['caption'][k] is None:
                    caption.append(None)
                else:
                    # caption.append(torch.stack([batch[i]['caption'][k] for i in range(len(batch))]))
                    feat_1_list = []
                    feat_2_list = []
                    for i in range(len(batch)):
                        feat_1, feat_2 = batch[i]['caption'][k]
                        feat_1_list.append(feat_1)
                        feat_2_list.append(feat_2)
                    feat_1_concat = torch.cat(feat_1_list, dim=0)
                    feat_2_concat = torch.cat(feat_2_list, dim=0)
                    caption.append((feat_1_concat, feat_2_concat))

        else:
            caption = [i['caption'] for i in batch]
        return batch_tubes, caption, batch_embed_scale, batch_conditions, batch_reference_frame, batch_face_mask, batch_ocr_mask

    def __call__(self, batch):
        batch_tubes, caption, batch_embed_scale, batch_conditions, batch_reference_frame, batch_face_mask, batch_ocr_mask = self.package(batch)

        pad_batch_tubes = torch.stack(batch_tubes)
        batch_reference_frame = torch.stack(batch_reference_frame)
        batch_conditions = None if any(item is None for item in batch_conditions) else torch.stack(batch_conditions)
        batch_face_mask = torch.stack(batch_face_mask)
        batch_ocr_mask = torch.stack(batch_ocr_mask)
        
        return pad_batch_tubes, caption, batch_embed_scale, batch_conditions, batch_reference_frame, batch_face_mask, batch_ocr_mask

    def process(self, batch_tubes, input_ids, cond_mask, t_ds_stride, ds_stride, ae_stride_thw):
        # pad to max multiple of ds_stride
        batch_input_size = [i.shape for i in batch_tubes]  # [(c t h w), (c t h w)]
        max_t, max_h, max_w = batch_input_size[0][1], batch_input_size[0][2], batch_input_size[0][3]
        #这里是确保是整数倍
...
class Collate2:
    ...
    def package(self, batch):
        ...
        batch_face_mask = [i['face_mask'] for i in batch]
        batch_ocr_mask = [i['ocr_mask'] for i in batch]
        batch_reference_pose = [i['reference_pose'] for i in batch]
        ...
        if isinstance(batch[0]['caption'], dict):
            ...
            caption = [i['caption'] for i in batch]
        return batch_tubes, caption, batch_embed_scale, batch_conditions, batch_reference_frame, batch_face_mask, batch_ocr_mask, batch_reference_pose

    def __call__(self, batch):
        batch_tubes, caption, batch_embed_scale, batch_conditions, batch_reference_frame, batch_face_mask, batch_ocr_mask, batch_reference_pose = self.package(batch)
        ...
        batch_face_mask = torch.stack(batch_face_mask)
        batch_ocr_mask = torch.stack(batch_ocr_mask)
        batch_reference_pose = torch.stack(batch_reference_pose)
        
        return pad_batch_tubes, caption, batch_embed_scale, batch_conditions, batch_reference_frame, batch_face_mask, batch_ocr_mask, batch_reference_pose
