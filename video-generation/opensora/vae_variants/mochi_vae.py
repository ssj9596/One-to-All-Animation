...
# from opensora.model_variants.attention_processor import Attention, MochiVaeAttnProcessor2_0
...
class MochiVAEWrapper(VAEWrapper):
    ...
    def encode(self, x):
        ...
        latents = (latents - latents_mean) / latents_std * self.vae.config.scaling_factor 
        return latents
...
def get_mochi_vae_wrapper(model_path, weight_dtype):
    from diffusers import AutoencoderKLMochi
    vae = AutoencoderKLMochi.from_pretrained(model_path, torch_dtype=weight_dtype)
    return MochiVAEWrapper(vae)
